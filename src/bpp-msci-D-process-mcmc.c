...
*/
   ...
   for (i = 0; i < n_mcmc_sample; i++) {
      for (j = 0; j < nmsci; j++)  p[j] = sample[i * nmsci + j];
      if (p[0] <= 0 || p[0] >= 1 || p[1] <= 0 || p[1] >= 1)
         printf("phi_xy[%5d] out of range\n", i + 1);
      else if (_D0DD1 && (p[4] <= 0 || p[4] >= 1 || p[5] <= 0 || p[5] >= 1))
         printf("phi_zw[%5d] out of range\n", i + 1);
      if (_D0DD1 == 0)
         towers[i] = (p[0] + p[1] > 1.0);
      else {
#if(1)
...
*/
   ...
   if (n_mcmc_sample <= 10000) print_towers(towers);
   for (i = 0, points_moved = 0; i < n_mcmc_sample; i++) points_moved += (towers[i] > 0);
   printf("\n%4d points reflected\n", points_moved);
...
int main(int argc, char* argv[])
{
  ...
  //char mcmcf[4096] = "../../A/0ngoing/bppMSci-twintowers/heliconius/h_mel_tim_num_noncod_mcmc_L500_r1.txt";
  char mcmcf[4096] = "../../A/0ngoing/bppMSci-twintowers/heliconius/h_mel_tim_num_exonic_mcmc_L500_r1.txt";
#elif msciD_data == 1  /* simulation-D-L500 */
